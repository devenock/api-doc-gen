...
> Implementation note: `Echo`, `Fiber`, and `Chi` route parsing currently **delegates to `parseGinRoutes`** because the call signatures are similar enough (`r.METHOD("path", handler)`). Group prefixes and auth-middleware detection are only built for `Gin/Echo/Fiber` (see `parseFile`).
...
├── api-doc-gen                # Pre-built ~12 MB darwin binary, COMMITTED to repo (see §11)
│
├── cmd/
...
- **Language:** Go. `go.mod` declares `go 1.24.3`. **However**:
  - `README.md` says **"Go 1.22+"**.
  - `Dockerfile` builds on `golang:1.22-alpine`.
  - `CONTRIBUTING.md` says **"Go 1.22 or later"**.
  - These three are inconsistent with `go.mod`. If you bump or constrain Go versions, update **all four** locations.
...
The `Dockerfile`:
1. **Build stage** — `golang:1.22-alpine`, `go mod download`, then `CGO_ENABLED=0 go build -ldflags="-s -w" -o /apidoc-gen .`.
...
Note: the `Dockerfile`'s base `golang:1.22-alpine` may fail on a `go.mod` declaring `go 1.24.3` (the toolchain directive must be satisfied). Bumping the Docker base image is a likely follow-up.
...
- **Pass 2 — `parseFile`**: re-parses `.go` files with comments. For Gin/Echo/Fiber, first builds:
  ...
  Then `ast.Inspect` dispatches per framework:
  - `parseGinRoutes` (also used for Echo, Fiber, Chi) — recognizes `selector.METHOD("path", handler)` for METHOD ∈ {GET,POST,PUT,DELETE,PATCH,HEAD,OPTIONS}.
  - `parseGorillaMethods` + `parseGorillaRoutes` — handles `r.HandleFunc("/x", h).Methods("POST")`. The `.Methods()` chain is parsed first to set the method on the most recently registered endpoint with the same path; if `.Methods()` is visited before `HandleFunc`, a new endpoint is appended.
  - `parseGenericRoutes` — last-resort generic `selector.METHOD("path", ...)` matcher when framework is `unknown`.

  For each endpoint, when the handler is a same-file ident:
...
- `PostmanGenerator` (`postman.go`):
  ...
  - **Known landmine:** `createPostmanURL` does `param.Example.(string)` for path params; `models.Parameter.Example` is `interface{}` and is **not set anywhere in the analyzer for path params** (they are created with `Schema{Type:"string"}` and no `Example`). Calling Postman generation on any endpoint that has a path parameter currently triggers a panic on this assertion. If you fix this, also add a regression test (none exist yet).
- `CustomGenerator` (`custom.go`):
  ...
  - Generates a markdown page per endpoint (`<method>-<dashed-path>.md`) inside a directory per group (tag or first path segment, title-cased via the deprecated `strings.Title` — replace with `cases.Title` from `golang.org/x/text/cases` if you bump Go versions strictly).
