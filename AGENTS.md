...
│   ├── TROUBLESHOOTING.md     # PATH issues, no-endpoints, invalid-type, CI prompts
...
| `-t, --type` | `""` (must end up as `swagger`/`postman`) | `type` |
...
5. `cfg.Validate()` — checks path exists and `DocType ∈ {swagger,postman}`; sets defaults.
...
`NewGenerator(docType, cfg)` switches on `docType` and returns one of two concrete generators. Each generator owns its own output-format conversion; **they do not share intermediate code beyond `models.APISpec`**.
...
- `PostmanGenerator` (`postman.go`):
  - Emits a single `collection.json` with `baseUrl` and `bearerToken` collection variables.
  ...
  - Sets `auth: {type: bearer, bearer: [{key: token, value: "{{bearerToken}}"}]}` on any endpoint whose `Security` field is non-empty.
...
- Errors if `DocType` is not one of `swagger`/`postman`.
...
1. Doc type (`Swagger/OpenAPI`, `Postman Collection`).
...
| `docs/TROUBLESHOOTING.md` | Users | PATH / no-endpoints / config-not-read / invalid-type / CI / verbose |
...
12. **`pkg/generator/postman.go`** uses `cases.Title(language.English).String(...)` from `golang.org/x/text/cases` for folder name casing (replacing the deprecated `strings.Title`).
